defmodule Solo.Vault do
  ...
  """
  @spec store(String.t(), String.t(), String.t(), String.t(), Keyword.t()) ::
          :ok | {:error, String.t()}
  def store(tenant_id, secret_name, secret_value, key, opts \\ [])
      when is_binary(tenant_id) and is_binary(secret_name) and is_binary(secret_value) and
             is_binary(key) do
  ...
  def retrieve(tenant_id, secret_name, key)
      when is_binary(tenant_id) and is_binary(secret_name) and is_binary(key) do
  ...
  @doc """
  Flush all secrets to disk (Phase 9 - for graceful shutdown).

  Ensures all secrets are persisted to disk.
  """
    ...
    result =
      try do
        secrets =
          ...
          |> Enum.filter(fn
            {{:secret, t, _name}, _value} -> t == tenant_id
            _ -> false
          end)
    ...
    result =
      try do
        case catch_flush(db) do
          :ok ->
            Logger.debug("[Vault] Flushed to disk")
            :ok

          error ->
            Logger.warning("[Vault] Flush failed: #{inspect(error)}")
            {:error, "Flush failed"}
        end
      rescue
        _e ->
  ...
  defp encrypt_secret(plaintext, key) do
    try do
      ...
      {ciphertext, tag} =
        :crypto.crypto_one_time_aead(:aes_256_gcm, derived_key, iv, plaintext, "", 16, true)
  ...
  defp decrypt_secret(combined, key) do
    try do
      ...
      plaintext =
        :crypto.crypto_one_time_aead(:aes_256_gcm, derived_key, iv, ciphertext, "", tag, false)
